// Ex0003  股票仿真系统 Wangxiaoyao Add  工具页签 - 计算器功能 - 提前还款 2021.8.24
...
void CDialogCalc04::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT3, m_iYearFirst);
	DDX_Text(pDX, IDC_EDIT4, m_iMonthFirst);
	DDX_Text(pDX, IDC_EDIT5, m_iYearAdvance);
	DDX_Text(pDX, IDC_EDIT6, m_iMonthAdvance);
	DDX_Text(pDX, IDC_EDIT9, m_dRateYear);
	DDX_CBString(pDX, IDC_COMBO1, m_strPaymentPattern);
	DDX_Text(pDX, IDC_EDIT7, m_dMoneyAdvancePayment);
	DDX_CBString(pDX, IDC_COMBO2, m_strProcessPattern);
	DDX_Text(pDX, IDC_EDIT8, m_dPayPerMonth);
	DDX_Text(pDX, IDC_EDIT17, m_dPaymentTotal);
	DDX_Text(pDX, IDC_EDIT18, m_dPayInterest);
	DDX_Text(pDX, IDC_EDIT19, m_dPayThisMonth);
	DDX_Text(pDX, IDC_EDIT21, m_dPayNew);
	DDX_Text(pDX, IDC_EDIT22, m_nInterestDecrease);
...
BEGIN_MESSAGE_MAP(CDialogCalc04, CDialogEx)
	ON_BN_CLICKED(IDC_BUTTON2, &CDialogCalc04::OnBnClickedButtonCalc)
