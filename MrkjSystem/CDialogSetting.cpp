...
void CDialogSetting::OnBnClickedButtonAdd()
{
	...
	if (DB.IsExistUsername(m_strUsername)) {
		CToolServices::MessageBoxFormat(_T("用户名'%s'己经存在"), m_strUsername.GetString());
		return;
	}
	...
	if (!DB.AddUser(user)) {
		CToolServices::MessageBoxFormat(_T("增加用户[%s]失败"), m_strUsername.GetString());
	}
	UpdateData(FALSE);
	UpdateListData();
}
...
void CDialogSetting::OnBnClickedButtonUpdate()
{
	...
	user.p010 = CStringServices::ToString(m_b010);
	if (!DB.UpdUser(user)) {
		CToolServices::MessageBoxFormat(_T("更改用户[%s]失败"), m_strUsername.GetString());
	}
	UpdateData(FALSE);
	UpdateListData();
}
...
void CDialogSetting::OnBnClickedButtonDelete()
{
	...
	if (!DB.DelUser(d.id.c_str())) {
		CToolServices::MessageBoxFormat(_T("删除用户[%s]失败"), m_strUsername.GetString());
	}
	UpdateListData();
}
