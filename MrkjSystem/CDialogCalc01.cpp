...
// Ex0003  股票仿真系统 Wangxiaoyao Add  工具页签-计算器功能-股票盈亏计算 2021.8.22
	...
	, m_fProfRate(0)
{
	...
	//pEdit->SetWindowText(_T("000001")); //设置默认显示的内容
	m_strCode = _T("000001");
}
...
void CDialogCalc01::OnEnKillfocusEdit1()
{
	...
	auto queryFlag = DB.QueryStockNameByCode(m_strCode, name);
	if (queryFlag) {
		m_strName = name.c_str();
	}
	else {
		m_strName = _T("");
		AfxMessageBox(_T("未查询到相关代号的股票"));
	}
	UpdateData(FALSE);
}
