...
// Ex0003  股票仿真系统 Wangxiaoyao Add  工具页签 - 计算器功能 - 能力评估 2021.8.24
...
void CDialogCalc03::OnBnClickedButtonCalc()
{
	...
	*/
	// 现有购房资金
	double d1 = GetDlgItemValueD(IDC_EDIT1);
	// 家庭月收入
	double d2 = GetDlgItemValueD(IDC_EDIT2);
	// 每月可用于购房支出
	double d3 = GetDlgItemValueD(IDC_EDIT3);
	// 计划还贷年限
	double d4 = GetDlgItemValueD(IDC_EDIT4);
	// 计划购房面积
	double d5 = GetDlgItemValueD(IDC_EDIT5);
	// 契税税率
	double d6 = GetDlgItemValueD(IDC_EDIT6);
	// 房屋公共维修基金收取比例
	double d7 = GetDlgItemValueD(IDC_EDIT7);
	// 检查个值的合法性
	{
		if (d4 < 1.0) {
