...
// Ex0003  股票仿真系统 Wangxiaoyao Add  数据库相关操作 2021.2.9

#include "pch.h"
#include "CMrkjDatabase.h"
...
//CMrkjDatabase CMrkjDatabase::DB;
...
CMrkjDatabase& CMrkjDatabase::GetInstance() {
	...
	, const char *password/*=nullptr*/)
{
	...
	try {
		...
		std::string connectionString = "driver={SQL Server};Server=" + string(host) +
			...
			string(username) + ";PWD=" +
			string(password) + ";";
		HRESULT hr = m_pConnection->Open(connectionString.c_str(), username, password, adModeUnknown);
		assert(SUCCEEDED(hr));
	}
	catch (_com_error &e) {
		CToolServices::MessageBoxFormat(_T("%s"), e.ErrorMessage());
		return false;
	}
	catch (...) {
		TRACE("发生异常\r\n");
...
bool CMrkjDatabase::MakeDayTable(const std::wstring &strStockCode)
{
	...
	_stprintf_s(sql, szFmt, wstrTableName.c_str(), wstrTableName.c_str(),
		CTableFiles::DATA_DATE, CTableFiles::PRICE_OPEN, CTableFiles::PRICE_CLOSE,
		CTableFiles::PRICE_MAX, CTableFiles::PRICE_MIN, CTableFiles::TURNOVER,
		CTableFiles::TRADING_VOLUME);
	try {
...
bool CMrkjDatabase::AddStock(const std::wstring &strStockCode, const std::wstring &strStockName)
{
	...
	try {
		...
		// 增加记录
		m_pRecordset->AddNew();
		m_pRecordset->PutCollect(CTableFiles::CODE, _variant_t(strStockCode.c_str()));
		m_pRecordset->PutCollect(CTableFiles::NAME, _variant_t(strStockName.c_str()));
		m_pRecordset->Update();
		m_pRecordset->Close();
...
bool CMrkjDatabase::UpdateStockNameByCode(const wchar_t *szCode, const wchar_t *szNewName)
{
	...
	try {
		...
		if (!m_pRecordset->adoEOF) {
			// 修改记录
			m_pRecordset->PutCollect(CTableFiles::NAME, _variant_t(szNewName));
			m_pRecordset->Update();
			bUpdated = true;
		}
		m_pRecordset->Close();
...
{
	assert(code);
	if (_tcslen(code) != CTableFiles::STOCK_LEN) {
		return false;
	}
...
bool CMrkjDatabase::AddDayData(const CDayDataEntry &data)
{
	...
	try {
		...
		rs->AddNew();
		// 设置各个列的值
		rs->PutCollect(CTableFiles::DATA_DATE, _variant_t(data.date.c_str()));
		rs->PutCollect(CTableFiles::PRICE_OPEN, _variant_t(data.open.c_str()));
		rs->PutCollect(CTableFiles::PRICE_CLOSE, _variant_t(data.close.c_str()));
		rs->PutCollect(CTableFiles::PRICE_MAX, _variant_t(data.max.c_str()));
		rs->PutCollect(CTableFiles::PRICE_MIN, _variant_t(data.min.c_str()));
		rs->PutCollect(CTableFiles::TURNOVER, _variant_t(data.turnover.c_str()));
		rs->PutCollect(CTableFiles::TRADING_VOLUME,
...
bool CMrkjDatabase::AddDayData(const VDayDataEntry &v)
{
	...
	try {
		...
		for each(auto data in v) {
			...
			// 增加记录
			m_pRecordset->AddNew();
			m_pRecordset->PutCollect(CTableFiles::DATA_DATE, _variant_t(data.date.c_str()));
			m_pRecordset->PutCollect(CTableFiles::PRICE_OPEN, _variant_t(data.open.c_str()));
			m_pRecordset->PutCollect(CTableFiles::PRICE_CLOSE, _variant_t(data.close.c_str()));
			m_pRecordset->PutCollect(CTableFiles::PRICE_MAX, _variant_t(data.max.c_str()));
			m_pRecordset->PutCollect(CTableFiles::PRICE_MIN, _variant_t(data.min.c_str()));
			m_pRecordset->PutCollect(CTableFiles::TURNOVER, _variant_t(data.turnover.c_str()));
			m_pRecordset->PutCollect(CTableFiles::TRADING_VOLUME, _variant_t(data.tradingVolume.c_str()));
			m_pRecordset->Update();
		}
		m_pRecordset->Close();
		m_pRecordset = nullptr;
	...
	const wchar_t *szDataEnd/* = nullptr*/)
{
	...
	try {
		...
		while (!rs->adoEOF) {
			CDayDataEntry obj;
			// 取出各列的值
			_variant_t date = rs->GetCollect(CTableFiles::DATA_DATE);
			_variant_t open = rs->GetCollect(CTableFiles::PRICE_OPEN);
			_variant_t close = rs->GetCollect(CTableFiles::PRICE_CLOSE);
			_variant_t max = rs->GetCollect(CTableFiles::PRICE_MAX);
			_variant_t min = rs->GetCollect(CTableFiles::PRICE_MIN);
			_variant_t turnover = rs->GetCollect(CTableFiles::TURNOVER);
			_variant_t tradingVolume = rs->GetCollect(CTableFiles::TRADING_VOLUME);
...
bool CMrkjDatabase::AddKind(const wchar_t *szName)
{
	...
	try {
		...
		CToolServices::MessageBoxFormat(L"%s", e.ErrorMessage());
		return false;
	}
	catch (...) {
		TRACE("发生异常\r\n");
